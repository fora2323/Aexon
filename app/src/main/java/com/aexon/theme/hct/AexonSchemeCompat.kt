...
class AexonSchemeCompat(@ColorInt seedColor: Int, private val isDark: Boolean) {
    ...
    fun colorPrimary(): Int {
        // Warna primary selalu ikut tone seed asli, sama persis di light & dark mode
        // (nggak ada penyesuaian "vivid" buat kontras).
        return primary.tone(seedTone.toInt())
    }
    ...
    @ColorInt
    fun colorSecondary(): Int = secondary.tone(max(40, seedTone.toInt()))
    @ColorInt
    ...
    @ColorInt
    fun colorTertiary(): Int = tertiary.tone(max(40, seedTone.toInt()))
    @ColorInt
