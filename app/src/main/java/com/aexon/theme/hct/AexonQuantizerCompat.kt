...
/**
* Ambil warna dominan dari array piksel bitmap (mis. wallpaper) buat jadi seed color.
* Piksel dikelompokkan per "bucket" warna (rounding ke kelipatan 16), lalu tiap bucket
* dinilai dari chroma (kejenuhan) + populasi piksel — mirip pendekatan Material You.
*/
object AexonQuantizerCompat {
	@ColorInt
	fun extractDominantColor(pixels: IntArray, @ColorInt fallbackColor: Int): Int {
		...
		// Skor = chroma tinggi + populasi besar; bucket dengan chroma < 15 diskip
		// kalau ada bucket lain, biar seed color nggak jatuh ke abu-abu/putih polos
		...
		// Fallback kalau semua bucket grayscale: ambil bucket dengan piksel terbanyak
		if (maxScore == -1.0) {
			var maxCount = 0
			for ((group, count) in colorCounts) {
				if (count > maxCount) {
