...
public class AexonFragmentActivity extends Fragment {
	...
	public void _loadApp(final double _app) {
		...
		isLoading = true;
		final boolean[] thisRunActive = {true};
		...
		new Thread(() -> {
			...
			for (int i = 0; i < total; i++) {
				...
				if (progress % 5 == 0 || progress == total) {
					if (getActivity() != null)
					getActivity().runOnUiThread(() -> loadingaexon1.setProgress(progress, total));
				}
			}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			final com.aexon.widget.AexonImageView icon_app = _view.findViewById(R.id.icon_app);
