...
private final Aexon.OnBinderDeadListener mainBinderDead = () -> {
	...
	private void initializeLogic() {
		...
		loadingaexon1.setStart(3000).setOnAexonLoadingListener(new AexonLoading.OnAexonLoadingListener() {
			@Override
	...
	public void _applyTheme(final AexonTheme _theme) {
		...
		aexonnavigationbbar1.setRippleColor(_theme.getColorOnSurface());
		
		fab_container.setBackground(new AexonDrawable.Builder(_theme.getColorPrimary()).cornerRadius(SketchwareUtil.getDimension(this, R.dimen.card_radius_medium)).ripple(_theme.getColorOnSurface()).build().build(this));
		fab_container.setClickable(true);
		fab_icon.setColorFilter(_theme.getColorOnPrimary(), PorterDuff.Mode.SRC_ATOP);
